...
Array.from(showProductImage)[0].onclick = function () {
  proImageid.src =
    "https://cdn.shopify.com/s/files/1/0399/1728/9633/products/Back-to-Balance-Cleanse-1_1024x1024.png?v=1595305403";
...
function showp(smallcartHim) {
  smallcartHim.forEach(function (el, index) {
    ...
    Array.from(itemsInCart)[0].innerHTML =
      "There are " + smallcartHim.length + " items in your Cart";
    ...
    Array.from(popProductName)[0].innerHTML =
      smallcartHim[smallcartHim.length - 1].Name;

    var popUpTotal = document.getElementsByClassName("popProductPrice");
    Array.from(popUpTotal)[0].innerHTML =
      smallcartHim[smallcartHim.length - 1].quant +
      " x " +
      "$" +
      smallcartHim[smallcartHim.length - 1].Price;
    // popupDiv
...
var descriptionListwhatsinIt = document.getElementById(
  "descriptionListwhatsinIt"
);
