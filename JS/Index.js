  ...
  kim.forEach(function (el) {
    ...
    addToCartButtun.onclick = function () {
      ...
      Array.from(popUpTotal)[0].innerHTML =
        store[store.length - 1].quant +
        " x " +
        "$" +
        store[store.length - 1].Price;
      // popupDiv
      ...
      if (store.length == 0) {
        showcartItemsNumber.style.visibility = "hidden";
...
var hoverforproductpageToshow = document.getElementById(
  "hoverforproductpageToshow"
);
...
document.querySelector(".smallCartTotalh4").innerHTML =
  "Total : $" + Ssum.toFixed(2);
